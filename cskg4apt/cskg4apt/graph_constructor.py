...
def create_graph_visualization(result: dict) -> str:
	...
	output_dir = os.path.join(os.getcwd(), "cskg4apt_output")
...
def create_cskg4apt_graph_visualization(kg_dict: dict) -> tuple:
	"""Create interactive graph visualization for CSKG4APT knowledge graph."""
	G = nx.DiGraph()
	...
	for entity in entities:
		...
		G.add_node(
			...
			type=entity_type,
			color=_ENTITY_COLORS.get(entity_type, _ENTITY_COLORS["default"]),
		)
	...
	# Calculate dynamic spring length
	max_edge_label_len = max(
		(len(d.get("relation", "")) for _, _, d in G.edges(data=True)), default=0
	)
	max_node_label_len = max(
		(len(d.get("text", "")) for _, d in G.nodes(data=True)), default=0
	)
	...
	# Add nodes
	for node_id, node_attrs in G.nodes(data=True):
		label_text = node_attrs.get("text", str(node_id))
		entity_type = node_attrs.get("type", "default")
		net.add_node(
			node_id,
			label=f"{label_text}\n({entity_type})",
			title=f"{label_text} [{entity_type}]",
			color=node_attrs.get("color", _ENTITY_COLORS["default"]),
			size=cfg["node_base_size"] + len(G[node_id]) * cfg["node_size_multiplier"],
			font={
				"size": cfg["node_font_size"],
	...
	for u, v, edge_attrs in G.edges(data=True):
		relation = edge_attrs.get("relation", "")
		source_text = G.nodes[u].get("text", "") if u in G.nodes else ""
		target_text = G.nodes[v].get("text", "") if v in G.nodes else ""
		...
		net.add_edge(
			...
			label=relation,
			title=relation,
			color=relation_colors.get(relation, cfg["extracted_edge_color"]),
			length=edge_length,
		)

	# Save graph
	timestamp = int(time.time() * 1000)
	file_name = f"cskg4apt_{timestamp}.html"
	output_dir = os.path.join(os.getcwd(), "cskg4apt_output")
	os.makedirs(output_dir, exist_ok=True)
	file_path = os.path.join(output_dir, file_name)
	...
	try:
		...
		logger.error(f"Error saving CSKG4APT graph: {e}")

	http_port = get_current_port()
	graph_url = f"http://localhost:{http_port}/{file_name}"
	logger.debug(f"CSKG4APT graph saved to {file_path}, accessible at {graph_url}")
