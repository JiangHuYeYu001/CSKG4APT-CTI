...
def is_our_server_responding(port: int, timeout: float = 1.0) -> bool:
	...
	try:
		...
		with urllib.request.urlopen(req, timeout=timeout) as response:
			# Check if it's serving from our cskg4apt_output directory
...
def find_free_port() -> int:
	...
	if is_our_server_responding(DEFAULT_PORT):
		logger.debug(f"Found existing CSKG4APT HTTP server on port {DEFAULT_PORT}")
		return DEFAULT_PORT
	...
	for port in range(DEFAULT_PORT + 1, DEFAULT_PORT + 20):
		if is_our_server_responding(port):
			logger.debug(f"Found existing CSKG4APT HTTP server on port {port}")
			return port
...
def _ensure_server_running():
	...
	network_dir_path = os.path.join(os.getcwd(), "cskg4apt_output")
	...
	class NetworkHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
		...
		def end_headers(self):
			...
			self.send_header("X-CSKG4APT-Server", "true")  # Marker to identify our server
			super().end_headers()
