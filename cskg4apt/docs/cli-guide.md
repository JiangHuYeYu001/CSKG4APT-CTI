# CSKG4APT Command Line Interface

CSKG4APT provides a powerful command line interface (CLI) for processing threat intelligence reports without the need for a graphical interface. This is ideal for automation, batch processing, and integration into existing security workflows.
...
cskg4apt --text "Your threat intelligence text here"

# Process a file
cskg4apt --input-file report.txt

# Process a CTI report URL
cskg4apt --url "https://example.com/report"

# Use specific models
cskg4apt --input-file report.txt --model gpt-4o --embedding-model text-embedding-3-large
```
...
| Option          | Short | Description                                                         |
| --------------- | ----- | ------------------------------------------------------------------- |
| `--output FILE` | `-o`  | Output file path (default: organized in cskg4apt/output/ directory) |

**Default output behavior:**

- For a file input: `cskg4apt/output/<input_file>_output.json`
- For a text input: `cskg4apt/output/output.json`
...
cskg4apt --input-file threat_report.txt

# Process text directly
cskg4apt --text "APT29 used PowerShell to download additional malware from 192.168.1.100"

# Process a web report directly from URL
cskg4apt --url "https://example.com/some-threat-report/"
```
...
cskg4apt -i report.txt --provider OpenAI --model gpt-4o --embedding-model text-embedding-3-large

# Use Gemini with default models
cskg4apt -i report.txt --provider Gemini

# Use AWS Claude
cskg4apt -i report.txt --provider AWS --model anthropic.claude-3-5-sonnet

# Use Ollama (local models)
cskg4apt -i report.txt --provider Ollama --model qwen2.5:7b --embedding-model nomic-embed-text

# Use Ollama with default models
cskg4apt -i report.txt --provider Ollama
```
...
cskg4apt -i report.txt --model gpt-4o --similarity-threshold 0.8
```

### Output Control

```bash
# Use organized default output (saves to cskg4apt/output/)
cskg4apt -i report.txt --model gpt-4o
# Creates: cskg4apt/output/report_output.json

# Specify custom output file
cskg4apt -i report.txt --output analysis_results.json
