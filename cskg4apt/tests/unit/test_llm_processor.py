"""Unit tests for cskg4apt.llm_processor module."""

from omegaconf import OmegaConf

from cskg4apt.llm_processor import (
	UrlSourceInput,
	UsageCalculator,
	_route_llm_call,
...
class TestRouteLLMCall:
	"""Test unified provider routing."""

	def test_custom_provider_routing(self, monkeypatch):
		"""Should route to custom endpoint when provider is Custom."""
		captured_kwargs = {}
		...
		monkeypatch.setattr("cskg4apt.llm_processor.litellm.completion", mock_completion)
	...
	def test_openai_provider_routing(self, monkeypatch):
		"""Should route OpenAI models without prefix."""
		captured_kwargs = {}
	...
	def test_aws_provider_routing(self, monkeypatch):
		"""Should prefix AWS models with bedrock/."""
		captured_kwargs = {}
		...
		response = _route_llm_call(
			"AWS", "anthropic.claude-3-7-sonnet",
			messages=[{"role": "user", "content": "test"}],
		)

		assert response == {"ok": True}
		assert captured_kwargs["model"] == "bedrock/anthropic.claude-3-7-sonnet"
...
class TestUrlSourceInput:
	...
	def test_url_source_input_success(self, monkeypatch):
		...
		monkeypatch.setattr("cskg4apt.llm_processor.trafilatura.fetch_url", mock_fetch_url)
		monkeypatch.setattr("cskg4apt.llm_processor.trafilatura.extract", mock_extract)
	...
	def test_url_source_input_fetch_failure(self, monkeypatch):
		...
		config = OmegaConf.create({})
		monkeypatch.setattr("cskg4apt.llm_processor.trafilatura.fetch_url", lambda *_args, **_kwargs: None)
	...
	def test_url_source_input_extraction_failure(self, monkeypatch):
		...
		config = OmegaConf.create({})
		monkeypatch.setattr("cskg4apt.llm_processor.trafilatura.fetch_url", lambda *_args, **_kwargs: "<html />")
		monkeypatch.setattr("cskg4apt.llm_processor.trafilatura.extract", lambda *_args, **_kwargs: None)
