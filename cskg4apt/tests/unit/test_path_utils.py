"""Unit tests for cskg4apt.utils.path_utils module."""

import os

from cskg4apt.utils.path_utils import BASE_DIR, resolve_path
...
class TestPathUtils:
	...
	def test_base_dir_is_cskg4apt(self):
		"""Test that BASE_DIR points to the cskg4apt package directory."""
		assert BASE_DIR.endswith("cskg4apt")

	def test_resolve_path_single_component(self):
		"""Test resolving a single path component."""
		result = resolve_path("config")
		assert os.path.isabs(result)
		assert result.endswith(os.path.join("cskg4apt", "config"))
	...
	def test_resolve_path_multiple_components(self):
		...
		assert os.path.isabs(result)
		assert result.endswith(os.path.join("cskg4apt", "config", "config.yaml"))
	...
	def test_resolve_path_with_nested_dirs(self):
		...
		assert result.endswith(os.path.join("cskg4apt", "data", "annotation", "test.json"))
