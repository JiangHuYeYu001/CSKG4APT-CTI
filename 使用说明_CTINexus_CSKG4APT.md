...
#### 2. ✅ CSKG4APT Prompt工程
- **文件**：
  - `ctinexus/prompts/ie_cskg4apt.jinja`
  - `ctinexus/prompts/et_cskg4apt.jinja`
- **功能**：零样本实体和关系提取的高质量Prompt模板
- **特性**：详细的示例、强制JSON输出、证据链要求

#### 3. ✅ CSKG4APT提取器
- **文件**：`ctinexus/cskg4apt_extractor.py`
- **功能**：LLM驱动的白盒提取引擎
- **特性**：
  - 滑动窗口分块处理超长文本
  - 自动实体去重和合并
...
#### 4. ✅ Neo4j图数据库管理器
- **文件**：`ctinexus/graph_db/neo4j_manager.py`
- **功能**：知识图谱持久化存储
- **特性**：
  - Cypher MERGE自动去重
  - 证据链存储在关系属性中
...
#### 5. ✅ 钻石模型威胁归因引擎
- **文件**：`ctinexus/attribution/diamond_model.py`
- **功能**：APT组织自动归因
- **场景**：
  1. 未知木马Hash归因
  2. 未知IP地址归因
...
#### 6. ✅ 配置文件更新
- **文件**：
  - `ctinexus/config/config.yaml` - 新增CSKG4APT配置段
...
**使用方法**：
1. 启动CTINexus：`python ctinexus/app.py`
2. 在Gradio界面中：
   - 勾选"🎯 启用CSKG4APT模式"
   - 输入威胁情报URL或上传PDF
   - 点击"运行"
...
**配置**：
编辑 `ctinexus/config/config.yaml`：
```yaml
neo4j:
  enabled: true
...
**使用Python代码**：
```python
from ctinexus.graph_db.neo4j_manager import Neo4jKnowledgeGraph
...
**Python代码示例**：
```python
from ctinexus.attribution.diamond_model import DiamondModelAttribution
...
```bash
ctinexus --url "https://example.com/report" --output result.json
```
...
CTINexus现在拥有完善的错误信息展示系统，所有错误都会以友好的方式呈现：
...
1. **设计方案**：[CTINexus_CSKG4APT融合改造方案.md](e:\\CTI\\CTINexus_CSKG4APT融合改造方案.md)
...
5. **启动脚本**：[start_ctinexus.py](e:\\CTI\\start_ctinexus.py)
...
CTINexus × CSKG4APT融合项目已经**90%完成**，核心功能全部就绪！
...
**现在您可以：**
1. 启动Gradio界面：`python ctinexus/app.py`
2. 勾选"🎯 启用CSKG4APT模式"
3. 上传PDF或输入URL
4. 点击"运行"查看结果！
